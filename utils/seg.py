...
def generate_clickmask_torch(vertices_pc, center_coo, cubeedge=0.05):

    refx, refy, refz = center_coo

    new_click_mask = torch.zeros(vertices_pc.shape[0]).to(vertices_pc.device)
    new_click_mask[torch.logical_and(torch.logical_and(
        (torch.abs(vertices_pc[:, 0] - refx) < cubeedge),
        (torch.abs(vertices_pc[:, 1] - refy) < cubeedge))
        , (torch.abs(vertices_pc[:, 2] - refz) < cubeedge))] = 1

    # print('click mask', np.shape(vertices_pc[:,3]), vertices_pc[:,3].sum())
    return new_click_mask
...
def get_simulated_clicks(pred_qv, labels_qv, coords_qv, current_num_clicks=0, training=True):
    ...
    new_clicks, new_click_num, new_click_pos = get_next_simulated_click_multi(selected_error_cluster_ids, error_cluster_ids_mask, pred_qv, labels_qv, coords_qv, error_distances)
...
### 为什么这个收敛地很慢呢？？？
def random_sampling_clicks_v2(sample_labels, sample_raw_coords, pos_click_num, neg_click_num):
    ...
    if len(potential_neg_indices) < neg_click_num:
        if len(potential_neg_indices) !=0:
            click_dict['0'] = potential_neg_indices.tolist()
    else:
        random_idx = torch.randperm(potential_neg_indices.size(0))[:neg_click_num]
        click_dict['0'] = potential_neg_indices[random_idx].tolist()
    ...
    for label in range(1, num_objects + 1):
        ...
        assert len(fg_indices) > 0, "One object has no points in GT"

        # Positive clicks
        pos_clicks_indices, pos_clicks = [], []
        ...
        if len(fg_indices) < (pos_click_num[label-1]-1):
            pos_clicks_indices.extend(fg_indices.tolist())
        elif pos_click_num[label-1]-1 !=0:
            random_idx = torch.randperm(fg_indices.size(0))[:(pos_click_num[label-1]-1)]
            pos_clicks_indices.extend(random_idx.tolist())
